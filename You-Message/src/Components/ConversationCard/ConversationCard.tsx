...
const ConversationCard: FC<IProps> = ({ conversationData }) => {
  ...
  useEffect(() => {
    ...
    const unsub2 = onSnapshot(lastMessageQuery, (snapShot) => {
      if (snapShot.size > 0) {
        ...
        const messages: IMessage[] = snapShot.docs.map(
          (message) => message.data() as IMessage
        );
        setUnreadCount(
          messages.filter(
            (message) =>
              message.status === "sent" && message.owner !== info?.uid
          ).length
        );
      } else {
        setLastMessage(null);
  ...
  const handleRightClick = (): void => {
    changeContextMenus([
      { icon: <DeleteIcon />, name: "Delete", function: deleteConversation },
    ]);
    openContext();
  };
  ...
  return (
    <Button
      ...
      className={classNames(
        "conversation-card",
        toUser.uid === currentConversation.toUser?.uid ? "active" : ""
      )}
    >
      <div className="conversation-card__content">
        ...
        <div className="conversation-card__info">
          ...
          <p className="conversation-card__last-message">
        ...
        <div className="conversation-card__status">
          ...
          {unreadCount > 0 && (
            <h5 className="conversation-card__notif">{unreadCount}</h5>
          )}
        </div>
      </div>
    </Button>
